...
class Welcome extends CI_Controller {
	...
	{
		$data['kategori'] = $this->db->query("select * from kategori where kategori_id!='1'")->result();
		$this->load->view('frontend/v_header');
		$this->load->view('frontend/v_daftar_vendor',$data);
		$this->load->view('frontend/v_footer');
	}

	public function daftar_vendor_act()
	{
		// form validasi
		$this->form_validation->set_rules('kategori','Kategori bisnis','required');
		$this->form_validation->set_rules('nama','Nama lengkap','required');
		$this->form_validation->set_rules('bisnis','Nama bisnis','required');
		$this->form_validation->set_rules('email','Email','required|is_unique[vendor.vendor_email]');
		...
		if($this->form_validation->run() != false){
			...
			$data = array(  
				'layanan_vendor' => $last_id,  
				'layanan_layanan' => $kategori
			);
			$this->m_data->insert_data($data,'layanan');

			redirect(base_url().'welcome/masuk_vendor?alert=terdaftar');
	...
	public function masuk_vendor()
	{
		$this->load->view('frontend/v_header');
		$this->load->view('frontend/v_masuk_vendor');
		$this->load->view('frontend/v_footer');
	}

	public function masuk_vendor_act()
	{
		// form validasi
		...
		if($this->form_validation->run() != false){
			...
			$login = $this->db->query("SELECT * FROM vendor WHERE vendor_email='$email' AND vendor_password='$password'");
			$cek = $login->num_rows();
			...
			if($cek > 0){
				...
				$_SESSION['vendor_status'] = "login";
				redirect(base_url()."vendor");
			}else{
				redirect(base_url()."welcome/masuk_vendor?alert=gagal");
			}
